...
class PeriodicTask(object):
    ...
    def start(self):
        _logger.debug('Start periodic task %s', self._exec_task)
        self._timer = Timer(0, self._exec_task, args=self._args)
        self._timer.start()

    def stop(self):
        _logger.debug('Stop periodic task %s', self._exec_task)
        self._timer.cancel()
